...
- 직렬화
  ...
  - 연관된 객체 중에 직렬화할 수 없는 것이 하나라도 있으면 직렬화 과정에서 예외가 발생한다.
  - 직렬화 할 때 어떤 객체를 건너뛰고 싶으면 transient 키워드를 사용하면 된다.
  - 역직렬화를 할 때는 그 JVM에서 해당 객체와 연관된 모든 객체의 클래스를 사용할 수 있어야 한다.
  - 객체를 읽을 때는 처음에 객체를 저장한 것과 같은 순서로 읽어오게 된다.
...
```
- 스레드
  - 스레드는 자바에서의 실행 스레드를 의미한다.
  - 자바에서는 스레드마다 각각의 호출 스택이 있다.
  - 대문자 T로 시작하는 Thread는 java.lang.Thread 클래스를 의미한다.
  - 스레드에서 처리할 작업은 Runnable 인터페이스를 구현하는 클래스의 인스턴스로 지정할 수 있다.
  - Runnable 인터페이스에서는 메서드가 run() 하나 밖에 없다.
  - Runnable 인터페이스에는 메서드가 하나뿐이기 때문에 Runnable 이 들어갈 자리에 람다 표현식을 쓸 수 있다.
  - 요즘은 멀티스레드 개발시 Executor 또는 ExecutorService를 사용한다.
  - 스레드가 아직 시작하지 않았으면 그 스레드는 아직 새 스레드 상태에 있다고 부른다.
  - 스레드를 시작하면 새로운 스택이 생성되며 Runnable 의 run()메서드가 스텍 맨 아래에 들어간다.
  - JVM의 스레드 스케줄러에 의해 현재 실행중인 스레드로 선택 받으면 그 스레드는 실행 중인 상태가 된다.
  - 스레드가 실행 중인 상태에서 봉쇄된 상태로 옮겨지는 경우도 있다.
  - 스레드 스케줄링은 모든 스레드가 공평하게 기회를 부여받을 수 있으리라는 가정을 해선 안된다.
  - Thread.sleep() 메서드에 전달된 인자로 지정한 시간 동안 스레드를 실행 중인 상태를 떠나게 된다.
  - sleep()메서드에서는 확인 예외를 던지기 때문에 try/catch로 감싸거나 예외를 선언해야 한다.
  - CountDownLatch 는 특정 이벤트가 정해진 횟수만큼 일어난 뒤에 작업을 다시 진행하도록 할 수 있다.
  - 스레드 풀은 스레드를 생성하고 재사용하고 없애는 작업을 처리해 준다.
  - shutdown() 은 쓰레드를 최대한 끝마치고 종료한다.
  - shutdownNow() 은 모든 스레드를 즉시 끝내고 종료한다.
18. 18강 : 동시성 이슈 처리 방법
  - 두 개 이상의 스레드가 동일한 데이터를 바꾸려고 하면 심각한 문제가 생길 수 있다.
  - 두 개 이상의 스레드에서 같은 개체를 엑세스하려고 하면 데이터 손상이 발생할 수 있다.
  - 스레드 안전성을 갖춘 객체를 만들려면 어떤 명령문들을 하나의 원자적인 절차로 묶어야 할지 결정해야 한다.
  - 두 스레드가 어떤 메서드에 같이 들어가지 않게 하고 싶다면 메서드 선언부에 synchronized 키워드를 추가한다.
  - 모든 객체에는 자물쇠가 하나씩 있고, 자물쇠에는 열쇠가 하나씩 있다. 
  - 객체에 동기화된 메서드가 있거나 특정 객채에서 synchronized 키워드를 사용하는 경우에는 자물쇠를 잘 챙겨야 한다.
  - 스레드가 동기화된 메서드에 돌어갈려고 할 때는 그 객체에 대한 열쇠를 받아야 한다. 
  - 다른 객체가 이미 열쇠를 가지고 있어서 열쇠가 없으면 그 스레드는 열쇠를 받을 수 있게 될 때까지 대기한다.
  - 여러 스레드에서 바꿀 수 있는 값이 하나 뿐이라면 CAS 연산이 있는 원자 변수를 쓸 수도 있다.
  - 대부분의 문제는 여러 스레드에서 데이터를 쓰는 경우에 발생하기 때문에 불변 데이터를 사용하는 것이 좋다.
  - CopyOnWriteArrayList<> 는 읽기는 많이 하지만, 쓰기는 자주 안하는 리스트일 때 스레드 안정성을 갖춘 자료구조다.